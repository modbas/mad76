/**
  * Mini-Auto-Drive MAD
  *
  * Copyright (C) 2017, Frank Traenkle, http://www.modbas.de
  *
  */

#include "mbmad/Spline.hpp"
#include "mbmad/TridiagonalSystem.hpp"
#include <cmath>
#include <cfloat>
#include <iostream>
  ...
  : breaks(breaks), vals(vals0.size()), segmentIds(segmentIds)
{
  ...
  for (std::size_t i = 0; i < vals0.size(); ++i) {
    vals[i][0] = vals0[i];
    vals[i][1] = vals1[i];
  }  
  computeCoefficients(bc);
}
...
int Spline::getNearestInternal(const std::array<float, dim>& y)
{
...
int Spline::getNearest(const std::array<float, dim>& y, float& x, float& dist)
{
  ...
  if (breaks.size() >= 2) {
    ...
    int imin1 = getNearestInternal(y);
...
int Spline::getNearestWaypoint(const std::array<float, dim>& y, float& x, float& dist)
{
  x = 0.0F;
  dist = 0.0F;
  int imin1 = getNearestInternal(y);
